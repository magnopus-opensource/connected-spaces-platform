...
class TestParser(unittest.TestCase):
    ...
    def test_parse_enum_fields(self):
        ...
        self.assertEqual(result.fields, [
            MetadataTypes.EnumFieldMetadata("Foo", None, []),
            MetadataTypes.EnumFieldMetadata("Bar", None, []),
            MetadataTypes.EnumFieldMetadata("Baz", None, [])
        ])
        self.assertEqual(result.is_flags, False)
        self.assertEqual(result.is_nested_type, False)
        self.assertEqual(result.doc_comments, None)
    ...
    def test_parse_enum_field_values(self):
        ...
        self.assertEqual(result.fields, [
            ...
            MetadataTypes.EnumFieldMetadata("Foo", "-1", []),
            MetadataTypes.EnumFieldMetadata("Bar", "0", []),
            MetadataTypes.EnumFieldMetadata("Baz", "1234", [])
        ])
        self.assertEqual(result.is_flags, False)
        self.assertEqual(result.is_nested_type, False)
        self.assertEqual(result.doc_comments, None)

    def test_parse_enum_with_comments(self):
        """ Test parsing an enum with doc comments. """
        wordreader = WordReader(
        """
        enum EnumName {
            /// This is Foo
        ...
        self.assertEqual(result.full_safe_type_name, "_EnumName")
        self.assertEqual(result.fields, [
            MetadataTypes.EnumFieldMetadata("Foo", None, ["This is Foo"]),
            MetadataTypes.EnumFieldMetadata("Bar", None, ["This is Bar"]),
            MetadataTypes.EnumFieldMetadata("Baz", None, ["This is Baz"])
        ])
        self.assertEqual(result.is_flags, False)
        self.assertEqual(result.is_nested_type, False)
