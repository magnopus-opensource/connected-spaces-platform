...
void CommandLineParser::ParseCommandLine(int argc, const char* argv[])
{
	try
	{
		...
		options.add_options()("u,User", "Email address for logging in to CHS", cxxopts::value<std::string>()->default_value("InvalidUser"))(
			"p,Password",
			"Password of the email address for logging in to CHS",
			cxxopts::value<std::string>()->default_value("InvalidPassword"))("e,Endpoint",
																			 "CHS Endpoint where the requested operation will be ran",
																			 cxxopts::value<std::string>()->default_value(DefaultEndpointBaseURI))(
			"t,Tenant",
			"CHS Tenant where the requested operation will be ran",
			cxxopts::value<std::string>()->default_value(DefaultTenant))("l,ListSpaces", "Show all spaces for the specified user")(
			"m,MigrateSpace",
			"Migrate space with the provided ID",
