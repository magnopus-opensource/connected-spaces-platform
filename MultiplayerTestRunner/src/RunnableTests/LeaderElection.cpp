...
void RunTest(csp::multiplayer::OnlineRealtimeEngine& RealtimeEngine)
{
    ...
    // Now listen for the elected callback
    ...
    RealtimeEngine.SetOnElectedScopeLeaderCallback(
            ...
            const csp::common::String&, const csp::common::String& LeaderUserId)
        {
            ...
            if (LeaderUserId == UserId)
            {
                ...
                if (*First)
                {
                    // We are the first client to get leadership, so call __PerformLeaderElectionInScope to ensure it doesnt go back to the first
                    // client
                    std::cout << "Performing leader election again, excluding the first user and itself: " << UserId << std::endl;

                    std::promise<csp::systems::NullResult> PerformLeaderElectionPromise;
                    std::future<csp::systems::NullResult> PerformLeaderElectionFuture = PerformLeaderElectionPromise.get_future();

                    MultiplayerSystem.__PerformLeaderElectionInScope(ScopeId, { { SpaceSystem.GetCurrentSpace().OwnerId, UserId } },
                        [&](const csp::systems::NullResult& Result) { PerformLeaderElectionPromise.set_value(Result); });

                    PerformLeaderElectionFuture.wait();
                }
                else
                {
