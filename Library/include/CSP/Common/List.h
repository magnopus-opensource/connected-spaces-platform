...
public:
	...
	List() : CurrentSize(0), MaximumSize(0), ObjectArray(nullptr)
	{
	...
	List(size_t MinimumSize) : CurrentSize(0), MaximumSize(0), ObjectArray(nullptr)
	{
	...
	List(const List<T>& Other) : CurrentSize(0), MaximumSize(0), ObjectArray(nullptr)
	{
	...
	CSP_NO_EXPORT List(std::initializer_list<T> List) : CurrentSize(0), MaximumSize(0), ObjectArray(nullptr)
	{
		if (List.size() == 0)
		{
			AllocList(LIST_DEFAULT_SIZE);
	...
	{
		return CurrentSize > 0 ? &ObjectArray[0] : nullptr;
	}
