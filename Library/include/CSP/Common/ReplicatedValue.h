...
public:
    ...
    CSP_NO_EXPORT template <class T> inline void Set(const T& inValue) { m_value = inValue; }
    // Internal templated getter.
    CSP_NO_EXPORT template <class T> inline const T& Get() const { return std::get<T>(m_value); }

    // Internal getter for variant.
    CSP_NO_EXPORT const ReplicatedValueImplType& GetValue() const { return m_value; };
    ...
    ReplicatedValueType GetReplicatedValueType() const { return m_replicatedType; }
...
private:
    CSP_START_IGNORE
    ReplicatedValueImplType m_value;
    CSP_END_IGNORE

    ReplicatedValueType m_replicatedType;
};
