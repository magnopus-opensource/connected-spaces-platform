...
public:
    ...
    /// @brief Creates a fiducial marker space component using the provided schema if it is compatible with the built-in schema.
...
private:
    FiducialMarkerSpaceComponent(const ComponentSchema& UpdatedSchema, csp::common::LogSystem* LogSystem, SpaceEntity* Parent);
};
