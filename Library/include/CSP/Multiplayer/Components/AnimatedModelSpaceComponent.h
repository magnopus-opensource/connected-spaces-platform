...
public:
    ...
    /// @brief Creates an animated model space component using the provided schema if it is compatible with the built-in schema.
...
private:
    AnimatedModelSpaceComponent(const schema::ComponentSchema& InSchema, csp::common::LogSystem* LogSystem, SpaceEntity* Parent);
};
