...
enum class LightPropertyKeys : uint16_t
{
    Name_DEPRECATED = 0,
    LightType,
    Color,
...
public:
    ...
    /// @brief Creates a light space component using the provided schema if it is compatible with the built-in schema.
...
private:
    LightSpaceComponent(const ComponentSchema& UpdatedSchema, csp::common::LogSystem* LogSystem, SpaceEntity* Parent);
};
