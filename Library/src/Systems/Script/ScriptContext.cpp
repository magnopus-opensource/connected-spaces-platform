...
ScriptContext::ScriptContext(ScriptSystem* InScriptSystem, qjs::Runtime* InRuntime, uint64_t InContextId)
	: ContextId(InContextId), TheScriptSystem(InScriptSystem), Runtime(InRuntime)
{
	Initialise();
}
...
uint64_t ScriptContext::GetId() const
{
	return ContextId;
}
...
size_t ScriptContext::GetNumImportedModules() const
{
	return Imports.size();
}
