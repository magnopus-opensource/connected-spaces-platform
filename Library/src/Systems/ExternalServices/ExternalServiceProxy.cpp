 ...
 */
#include "CSP/Systems/ExternalServices/ExternalServiceProxy.h"
...
void AgoraUserTokenResult::OnResponse(const services::ApiResponseBase* ApiResponse)
{
    ...
    if (ApiResponse->GetResponseCode() == services::EResponseCode::ResponseSuccess)
    {
        ...
        if (!OperationResult)
        {
            CSP_LOG_MSG(csp::common::LogLevel::Error, "AgoraUserTokenResult invalid");
        ...
        {
            CSP_LOG_MSG(csp::common::LogLevel::Error, "AgoraUserTokenResult doesn't contain expected member: token");
