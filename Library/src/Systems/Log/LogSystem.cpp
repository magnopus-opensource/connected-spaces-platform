...
void LogSystem::SetLogCallback(LogCallbackHandler InLogCallback)
{
	Callbacks->LogCallback = InLogCallback;
}

void LogSystem::SetEventCallback(EventCallbackHandler InEventCallback)
{
	Callbacks->EventCallback = InEventCallback;
}

void LogSystem::SetBeginMarkerCallback(BeginMarkerCallbackHandler InBeginCallback)
{
	Callbacks->BeginMarkerCallback = InBeginCallback;
}

void LogSystem::SetEndMarkerCallback(EndMarkerCallbackHandler InEndCallback)
{
	Callbacks->EndMarkerCallback = InEndCallback;
}

void LogSystem::SetSystemLevel(const csp::systems::LogLevel InSystemLevel)
{
	SystemLevel = InSystemLevel;
}

csp::systems::LogLevel LogSystem::GetSystemLevel()
{
	return SystemLevel;
}

bool LogSystem::LoggingEnabled(const csp::systems::LogLevel Level)
{
...
void LogSystem::LogToFile(const csp::common::String& InMessage)
{
	CSP_LOG(InMessage.c_str());
}

void LogSystem::ClearAllCallbacks()
{
	Callbacks->Clear();
}
