        ...
        this->MultiplayerSystem->GetScopesBySpace(SpaceResult.GetSpace().Id)
            .then(async::inline_scheduler(),
                [this, FinishedGetScopeEvent, SpaceResult, RealtimeEngine](const csp::systems::ScopesResult& ScopesResult)
                {
                    ...
                    // The default-generated scope always contains an id + "_autoGenerated".
                    // With newer spaces, the id always seems to be the space id. However, with older ones, it seems to be a random id.
                    // So to account for this, we check if the name contains "_autoGenerated" to identify the default scope.
                    // This is NOT ideal, but we haven't fully implemented scopes at this point, and we need a way of getting the default scope.
                    auto DefaultScopeIt = std::find_if(ScopesResult.GetScopes().begin(), ScopesResult.GetScopes().end(),
                        [](const Scope& Scope) { return Scope.Name.Contains("_autoGenerated"); });
