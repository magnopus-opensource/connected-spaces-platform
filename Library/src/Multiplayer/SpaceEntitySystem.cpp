...
uint64_t ParseGenerateObjectIDsResult(const signalr::value& Result)
{
	...
	if (Result.is_array())
	{
		...
		for (const signalr::value& IdValue : Ids)
		{
			...
			{
				FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Verbose, "Entity Id=%i", IdValue.as_uinteger());
...
SpaceEntityEventHandler::SpaceEntityEventHandler(SpaceEntitySystem* EntitySystem) : EntitySystem(EntitySystem)
{
}
									 ...
									 EntityCreatedCallback Callback)
{
	const std::function<void(signalr::value, std::exception_ptr)> LocalIDCallback
		= [this, InName, InSpaceTransform, InState, InAvatarId, InAvatarPlayMode, Callback](const signalr::value& Result,
																							const std::exception_ptr& Except)
	{
		...
		{
			FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error, "Failed to generate object ID. Exception: %s", e.what());
			Callback(nullptr);
		}
		...
		const std::function LocalSendCallback = [this, Callback, NewAvatar](const signalr::value& /*Result*/, const std::exception_ptr& Except)
		{
			...
			{
				FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error, "Failed to create Avatar. Exception: %s", e.what());
				Callback(nullptr);
			}
...
void SpaceEntitySystem::CreateObject(const csp::common::String& InName, const SpaceTransform& InSpaceTransform, EntityCreatedCallback Callback)
{
	const std::function LocalIDCallback = [this, InName, InSpaceTransform, Callback](const signalr::value& Result, const std::exception_ptr& Except)
	{
			...
			= [this, Callback, NewObject](const signalr::value& /*Result*/, const std::exception_ptr& Except)
		{
			...
			{
				FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error, "Failed to create object. Exception: %s", e.what());
				Callback(nullptr);
			}
...
void SpaceEntitySystem::DestroyEntity(SpaceEntity* Entity, CallbackHandler Callback)
{
	const std::function LocalCallback = [this, Callback](const signalr::value& /*EntityMessage*/, const std::exception_ptr& Except)
	{
		...
		{
			FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error, "Failed to destroy entity. Exception: %s", e.what());
			Callback(false);
		}
...
void SpaceEntitySystem::SetEntityCreatedCallback(EntityCreatedCallback Callback)
{
	...
	{
		FOUNDATION_LOG_WARN_MSG("SpaceEntityCreatedCallback has already been set. Previous callback overwritten.");
	}
...
void SpaceEntitySystem::SetInitialEntitiesRetrievedCallback(CallbackHandler Callback)
{
	...
	{
		FOUNDATION_LOG_WARN_MSG("InitialEntitiesRetrievedCallback has already been set. Previous callback overwritten.");
	}
...
void SpaceEntitySystem::SetScriptSystemReadyCallback(CallbackHandler Callback)
{
	...
	{
		FOUNDATION_LOG_WARN_MSG("ScriptSystemReadyCallback has already been set. Previous callback overwritten.");
	}
...
void SpaceEntitySystem::BindOnObjectMessage()
{
	Connection->On("OnObjectMessage",
				   [this](const signalr::value& Params)
				   {
					   ...
					   {
						   FOUNDATION_LOG_WARN_MSG("Called SpaceEntityCreatedCallback without it being set! Call SetEntityCreatedCallback first!");
					   }
				   });
}
...
void SpaceEntitySystem::QueueEntityUpdate(SpaceEntity* EntityToUpdate)
{
		...
		&& EntityToUpdate->TransientDeletionComponentIds.Size() == 0)
	{
		...
		// FOUNDATION_LOG_MSG(csp::systems::LogLevel::Log, "Skipped patch message send as no data changed");
		return;
	}
...
bool SpaceEntitySystem::IsLeaderElectionEnabled() const
{
	return (ElectionManager != nullptr);
}
...
ComponentBase* SpaceEntitySystem::FindComponentById(uint16_t Id)
{
	...
	FOUNDATION_LOG_ERROR_FORMAT("FindComponentById: Component with id: %s doesn't exist!", std::to_string(Id).c_str());
...
void SpaceEntitySystem::RunScriptRemotely(int64_t ContextId, const csp::common::String& ScriptText)
{
	...
	FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::VeryVerbose, "RunScriptRemotely Script='%s'", ScriptText.c_str());
...
void SpaceEntitySystem::LockEntityUpdate() const
{
	EntitiesLock->lock();
}

void SpaceEntitySystem::UnlockEntityUpdate() const
{
	EntitiesLock->unlock();
}
	...
	const std::function LocalCallback = [](const signalr::value& /*Result*/, const std::exception_ptr& Except)
	{
		...
		catch (const std::exception& e)
		{
			FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error,
								  "Failed to send list of entity update due to a signalr exception! Exception: %s",
								  e.what());
		}
	};
...
void SpaceEntitySystem::AddPendingEntity(SpaceEntity* EntityToAdd)
{
	...
	{
		FOUNDATION_LOG_ERROR_MSG("Attempted to add a pending entity that we already have!");
	}
}
...
void SpaceEntitySystem::ApplyIncomingPatch(const signalr::value* EntityMessage)
{
	...
	Deserialiser.EnterEntity();
	{
		...
		else
		{
			...
			if (!EntityFound)
			{
				FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error, "Failed to find an entity with ID %d when recieved a patch message.", EntityID);
			}
		}
	}
...
void SpaceEntitySystem::HandleException(const std::exception_ptr& Except, const std::string& ExceptionDescription)
{
	...
	{
		FOUNDATION_LOG_FORMAT(csp::systems::LogLevel::Error, "%s Exception: %s", ExceptionDescription.c_str(), e.what());
	}
}
} // namespace csp::multiplayer
