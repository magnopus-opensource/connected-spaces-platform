...
void SetWebSocketClient(IWebSocketClient* InCSPWebSocketClientPtr)
{
	CSPWebSocketClientPtr = InCSPWebSocketClientPtr;
}


CSPWebsocketClient::CSPWebsocketClient() noexcept : UserSystem(nullptr), LoginState(nullptr)
{
...
void CSPWebsocketClient::start(const std::string& url, std::function<void(std::exception_ptr)> callback)
{
	...
	{
		ok ? callback(nullptr) : callback(std::make_exception_ptr(std::runtime_error("Socket Start Error")));
	};
...
void CSPWebsocketClient::stop(std::function<void(std::exception_ptr)> callback)
{
	IWebSocketClient::CallbackHandler LocalCallback = [callback](bool ok)
	{
		ok ? callback(nullptr) : callback(std::make_exception_ptr(std::runtime_error("Socket Stop Error")));
	};
...
public:
	...
	bool WaitFor(std::function<bool(void)> IsDone,
...
void CSPHttpClient::send(const std::string& url, const http_request& request, std::function<void(const http_response&, std::exception_ptr)> callback)
{
	...
	WebClientHttps->SendRequest(csp::web::ERequestVerb::POST,
								csp::web::Uri(UriString.c_str()),
								PayLoad,
								&SignalRConnectionReceiver,
