...
SignalRConnection::SignalRConnection(const std::string& BaseUri, const uint32_t KeepAliveSeconds, std::shared_ptr<websocket_client> WebsocketClient)
	: Connection(hub_connection_builder::create(BaseUri)
					 .with_http_client_factory(
						 ...
						 {
							 return std::make_shared<CSPHttpClient>();
						 })
					 .with_websocket_factory(
						 [WebsocketClient](const signalr_client_config&)
						 {
...
void SignalRConnection::Start(std::function<void(std::exception_ptr)> Callback)
{
...
SignalRConnection::ConnectionState SignalRConnection::GetConnectionState() const
{
	return (ConnectionState) Connection.get_connection_state();
}

std::string SignalRConnection::GetConnectionId() const
{
	return Connection.get_connection_id();
}
...
void SignalRConnection::Invoke(const std::string& MethodName,
