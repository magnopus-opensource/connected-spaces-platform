...
uint64_t SpaceEntity::GetId() const
{
	return Id;
}
...
const csp::common::Vector4& SpaceEntity::GetRotation() const
{
...
bool SpaceEntity::GetIsTransient() const
{
	return !IsPersistant;
}

const csp::common::String& SpaceEntity::GetThirdPartyRef() const
{
...
const csp::systems::EThirdPartyPlatform SpaceEntity::GetThirdPartyPlatformType() const
{
	return ThirdPartyPlatform;
}

SpaceEntityType SpaceEntity::GetEntityType() const
{
	return Type;
}
...
void SpaceEntity::InsertChildBefore(uint64_t ChildId, uint64_t NextChildId)
{
	const auto& Entity = EntitySystem->FindSpaceEntityById(ChildId);

	if (Entity)
	{
		Entity->SetNextEntityId(NextChildId);
		Entity->SetParentId(GetId());
	}
	else
	{
...
void SpaceEntity::SetUpdateCallback(UpdateCallback Callback)
{
	EntityUpdateCallback = Callback;
}

void SpaceEntity::SetDestroyCallback(DestroyCallback Callback)
{
	EntityDestroyCallback = Callback;
}

void SpaceEntity::SetPatchSentCallback(CallbackHandler Callback)
{
	EntityPatchSentCallback = Callback;
}
...
const csp::common::Map<uint16_t, ComponentBase*>* SpaceEntity::GetComponents() const
{
	return &Components;
}
...
void SpaceEntity::SerialisePatch(IEntitySerialiser& Serialiser) const
{
	...
	Serialiser.BeginEntity();
	{
		...
		Serialiser.BeginComponents();
		{
			...
			if (DirtyProperties.HasKey(COMPONENT_KEY_VIEW_NEXT_ENTITY_ID))
			{
				Serialiser.AddViewComponent(COMPONENT_KEY_VIEW_NEXT_ENTITY_ID, DirtyProperties[COMPONENT_KEY_VIEW_NEXT_ENTITY_ID].GetInt());
			}
...
void SpaceEntity::Deserialise(IEntityDeserialiser& Deserialiser)
{
	...
	Deserialiser.EnterEntity();
	{
		...
		Deserialiser.EnterComponents();
		{
			...
			{
				NextEntityId = Deserialiser.GetViewComponent(COMPONENT_KEY_VIEW_NEXT_ENTITY_ID).GetInt();
			}
		}
...
EntityScript* SpaceEntity::GetScript()
{
	return Script;
}

bool SpaceEntity::IsSelected() const
{
	return SelectedId != 0;
}

uint64_t SpaceEntity::GetSelectingClientID() const
{
	return SelectedId;
}
...
void SpaceEntity::AddChildEntitiy(SpaceEntity* ChildEntity)
{
	...
	ChildEntity->PrevEntity = PreviousChildEntity;
...
void SpaceEntity::AddChildEntitiyBefore(SpaceEntity* ChildEntity, SpaceEntity* NextEntity)
{
	...
	{
		// NextEntity is currently the first in the list, so update our first
		FirstChild = this;
	}

	NextEntity->PrevEntity	= ChildEntity;
	ChildEntity->NextEntity = NextEntity;
...
void SpaceEntity::UpdateEntityPosition()
{
	...
	SpaceEntity* NewParent = EntitySystem->FindSpaceEntityById(ParentId);
...
SpaceEntity* SpaceEntity::GetLastChild() const
{
	...
	while (Current && Current->NextEntity)
	{
		Current = Current->NextEntity;
	}
...
void SpaceEntity::SetNextEntityId(uint64_t Value)
{
	...
	if (NextEntity == nullptr || NextEntity->GetId() != Value)
	{
...
{
	EntitySystem->ClaimScriptOwnership(this);
}
...
void SpaceEntity::AddRef()
{
	++(*RefCount);
}

void SpaceEntity::RemoveRef()
{
	--(*RefCount);
}

std::atomic_int* SpaceEntity::GetRefCount()
{
	return RefCount;
}
