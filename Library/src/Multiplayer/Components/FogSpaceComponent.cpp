...
{
	if (const auto& RepVal = GetProperty(static_cast<uint32_t>(FogPropertyKeys::Position));
...
{
	if (const auto& RepVal = GetProperty(static_cast<uint32_t>(FogPropertyKeys::Rotation));
...
{
	if (const auto& RepVal = GetProperty(static_cast<uint32_t>(FogPropertyKeys::Scale));
		RepVal.GetReplicatedValueType() == ReplicatedValueType::Vector3)
	{
		return RepVal.GetVector3();
	}
