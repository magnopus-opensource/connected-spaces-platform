...
bool ConversationSystemHelpers::StringToBool(const String& value)
{
	return (value == "true") ? true : false;
}

String ConversationSystemHelpers::BoolToString(const bool value)
{
...
{
	List<String> StringList = value.Split(',');
	return Vector4(std::stof(StringList[0].c_str()),
				   std::stof(StringList[1].c_str()),
				   std::stof(StringList[2].c_str()),
				   std::stof(StringList[3].c_str()));
}
...
String ConversationSystemHelpers::SpaceTransformToString(const SpaceTransform& value)
{
	return StringFormat("%s|%s|%s",
						Vector3ToString(value.Position).c_str(),
						Vector4ToString(value.Rotation).c_str(),
						Vector3ToString(value.Scale).c_str());
}
