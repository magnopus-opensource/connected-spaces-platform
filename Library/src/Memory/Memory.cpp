...
{
	return csp::memory::DefaultAllocator()->Allocate(size);
}

void* operator new(size_t size, std::align_val_t alignment)
{
	return csp::memory::DefaultAllocator()->Allocate(size, alignment);
}
...
{
	csp::memory::Deallocate(Ptr);
}

void operator delete(void* Ptr, size_t size)
{
	csp::memory::Deallocate(Ptr, size);
}
void operator delete[](void* Ptr)
{
	csp::memory::Deallocate(Ptr);
...
{
	csp::memory::Deallocate(Ptr, Allocator);
}
