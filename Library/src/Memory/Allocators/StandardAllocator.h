...
template <typename TLockTrait> StandardAllocator<TLockTrait>::StandardAllocator() : AllocatedBytes(0)
{
}
...
template <typename TLockTrait> void* StandardAllocator<TLockTrait>::Allocate(size_t n)
{
	return Allocate(n, CSP_ALLOCATOR_MIN_ALIGNMENT);
}
...
template <typename TLockTrait> const size_t StandardAllocator<TLockTrait>::GetAllocatedBytes() const
{
