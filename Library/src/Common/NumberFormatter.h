...
{
public:
	NumberFormatterPtr(char* InPtr, std::size_t InOffset) : Beg(InPtr), Cur(InPtr), End(InPtr + InOffset)
	{
	}
	...
	char*& operator+=(const int Incr)
	{
		return Cur += Incr;
	}

	char*& operator-=(const int Decr)
	{
		return Cur -= Decr;
	}
...
private:
	template <typename T>
	static bool
		IntToStr(T Value, unsigned short Base, char* Result, std::size_t& Size, bool Prefix = false, int Width = -1, char Fill = ' ', char ThSep = 0)
	...
	template <typename T>
	static bool
		UIntToStr(T Value, unsigned short Base, char* Result, std::size_t& Size, bool Prefix = false, int Width = -1, char Fill = ' ', char ThSep = 0)
