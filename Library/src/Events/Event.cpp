...
private:
	...
	struct EventParam
	{
		EventParam(EParamType Type) : ParamType(Type)
		{
		}
...
{
	Impl->AddInt(Key, Value);
}

void Event::AddString(const char* Key, const char* Value)
{
	Impl->AddString(Key, Value);
}

void Event::AddFloat(const char* Key, const float Value)
{
	Impl->AddFloat(Key, Value);
}

void Event::AddBool(const char* Key, const bool Value)
{
	Impl->AddBool(Key, Value);
}

const int Event::GetInt(const char* Key) const
{
	return Impl->GetInt(Key);
}

const char* Event::GetString(const char* Key) const
{
	return Impl->GetString(Key);
}

const float Event::GetFloat(const char* Key) const
{
	return Impl->GetFloat(Key);
}

bool Event::GetBool(const char* Key) const
{
