...
HttpRequest::HttpRequest(WebClient* InClient,
						 ERequestVerb InVerb,
						 const csp::web::Uri& InUri,
						 HttpPayload& InPayload,
						 IHttpResponseHandler* ResponseCallback,
						 csp::common::CancellationToken& CancellationToken,
						 bool CallbackIsAsync)
	: Client(InClient)
	, Verb(InVerb)
	, Uri(InUri)
	, Payload(InPayload)
...
const ERequestVerb HttpRequest::GetVerb() const
{
	return Verb;
}

const csp::web::Uri& HttpRequest::GetUri() const
{
	return Uri;
}
...
IHttpResponseHandler* HttpRequest::GetCallback() const
{
	return Callback;
}

bool HttpRequest::GetIsCallbackAsync() const
{
	return IsCallbackAsync;
}

void HttpRequest::SetResponseCode(EResponseCodes InReponseCode)
{
	Response.SetResponseCode(InReponseCode);
}

void HttpRequest::SetResponseData(const char* Data, size_t DataLength)
{
	Response.GetMutablePayload().SetContent(Data, DataLength);
}

void HttpRequest::AllocateResponseData(size_t DataLength)
{
	Response.GetMutablePayload().AllocateContent(DataLength);
}
...
float HttpRequest::GetRequestProgressPercentage() const
{
	return Progress.GetProgressPercentage();
}

float HttpRequest::GetResponseProgressPercentage() const
{
	return Response.GetProgress().GetProgressPercentage();
}
...
HttpProgress& HttpRequest::GetProgress()
{
	return Progress;
}
...
void HttpRequest::IncRefCount()
{
	++RefCount;
}

uint32_t HttpRequest::DecRefCount()
{
	return --RefCount;
}

uint32_t HttpRequest::GetRefCount() const
{
	return RefCount;
}

uint32_t HttpRequest::GetRetryCount() const
{
	return RetryCount;
}

void HttpRequest::SetSendDelay(const std::chrono::milliseconds InSendDelay)
{
	SendDelay = InSendDelay;
}

const std::chrono::milliseconds HttpRequest::GetSendDelay()
{
	return SendDelay;
}

void HttpRequest::EnableAutoRetry(bool Enable)
{
	IsAutoRetryEnabled = Enable;
}
...
bool HttpRequest::Cancelled()
{
	return CancellationToken->Cancelled();
}

void HttpRequest::RefreshAccessToken()
{
	Payload.RefreshBearerToken();
}
