...
```
LogSystem.LogMsg(csp::common::LogLevel::Error, "Critical error encountered.");
```
...
```
LogSystem.LogMsg(csp::common::LogLevel::Warning, "Potential issue detected.");
```   
...
```
LogSystem.LogMsg(csp::common::LogLevel::Info, "System operation details.");
```   

4. **Verbose:** It represents detailed information useful for debugging. Messages are more verbose and are typically used during development or troubleshooting.

```
LogSystem.LogMsg(csp::common::LogLevel::Verbose, "Debugging details.");
```   
...
Use the SetSystemLevel function to set the system's log level. This determines the verbosity of logs generated by the system. Higher verbosity levels include all lower levels, meaning setting a higher log level, like Verbose, will also capture logs from Error, Warning, and Info levels. For example: 
...
```
csp::common::LogLevel currentLevel = LogSystem.GetSystemLevel();
```
...
```
bool isLogging = LogSystem.LoggingEnabled(csp::common::LogLevel::Info);
```
