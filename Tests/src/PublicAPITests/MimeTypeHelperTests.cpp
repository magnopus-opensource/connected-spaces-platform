...
CSP_PUBLIC_TEST(CSPEngine, MimeTypeHelperTests, AccessMimeTypesTest)
{
	EXPECT_EQ(MimeTypeHelper::Get().MimeType.IMAGE_JPEG, "image/jpeg");
}
#endif

#if RUN_ALL_UNIT_TESTS || RUN_MIMETYPEHELPER_TESTS || RUN_MIMETYPEHELPER_ACCESS_FILEEXTENSIONS_TEST
CSP_PUBLIC_TEST(CSPEngine, MimeTypeHelperTests, AccessFileExtensionsTest)
{
	EXPECT_EQ(MimeTypeHelper::Get().FileExtension.JPEG, "jpeg");
}
#endif
