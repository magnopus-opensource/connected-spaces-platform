...
//     EXPECT_EQ(Result.GetResultCode(), csp::systems::EResultCode::Success);
//
//     auto [Result2] = AWAIT_PRE(UserSystem, UserSystem::ForgotPassword, RequestPredicate, "testnopus.pokemon@magnopus.com", nullptr, nullptr,
//     false);
//
...
//
//     auto [Result4] = AWAIT_PRE(UserSystem, UserSystem::ForgotPassword, RequestPredicate, "testnopus.pokemon+1@magnopus.com", nullptr, nullptr,
//     true);
//
...
//
//	// Retrieve Authorise URL for Google
//	auto [Result] = AWAIT_PRE(UserSystem, GetThirdPartyProviderAuthoriseURL, RequestPredicate,
//csp::systems::EThirdPartyAuthenticationProviders::Google, RedirectURL); 	EXPECT_EQ(Result.GetResultCode(), csp::systems::EResultCode::Success);
//
...
//	// 2. Navigate to the AuthoriseURL in a browser, but make sure that for the Third party account you're using there's already a created CHS
//account (same email address)
...
//
//	auto [LoginResult] = AWAIT_PRE(UserSystem, LoginToThirdPartyProvider, RequestPredicate,
//csp::systems::EThirdPartyAuthenticationProviders::Google, RedirectURL, csp::common::String(GoogleToken.c_str()),
//csp::common::String(StateId.c_str())); 	EXPECT_EQ(LoginResult.GetResultCode(), csp::systems::EResultCode::Success); 	const auto UserId =
//LoginResult.GetLoginState()->UserId;
//
...
//	// Retrieve Authorise URL for Google
//	auto [Result] = AWAIT_PRE(UserSystem, GetThirdPartyProviderAuthoriseURL, RequestPredicate,
//csp::systems::EThirdPartyAuthenticationProviders::Discord, RedirectURL); 	EXPECT_EQ(Result.GetResultCode(), csp::systems::EResultCode::Success);
//
//	//retrieve the StateId from the URL
//	const auto& AuthoriseURL = Result.GetValue();
//	const auto Tokens = AuthoriseURL.Split('&');
...
//
//	auto [LoginResult] = AWAIT_PRE(UserSystem, LoginToThirdPartyProvider, RequestPredicate,
//csp::systems::EThirdPartyAuthenticationProviders::Discord, RedirectURL, csp::common::String(DiscordToken.c_str()),
//csp::common::String(StateId.c_str())); 	EXPECT_EQ(LoginResult.GetResultCode(), csp::systems::EResultCode::Success); 	const auto UserId =
//LoginResult.GetLoginState()->UserId;
//
//	// test that we are in fact logged in
...
//	// Retrieve Authorise URL for Apple
//	auto [Result] = AWAIT_PRE(UserSystem, GetThirdPartyProviderAuthoriseURL, RequestPredicate,
//csp::systems::EThirdPartyAuthenticationProviders::Apple, RedirectURL); 	EXPECT_EQ(Result.GetResultCode(), csp::systems::EResultCode::Success);
//
//	//retrieve the StateId from the URL
//	const auto& AuthoriseURL = Result.GetValue();
//	const auto Tokens = AuthoriseURL.Split('&');
...
//     auto [Result]
//         = Awaitable(&csp::systems::UserSystem::Login, UserSystem, "", TestUser.Email, GeneratedTestAccountPassword,
//         nullptr).Await(RequestPredicate);
//
//     EXPECT_EQ(Result.GetResultCode(), csp::systems::EResultCode::Success);
//
