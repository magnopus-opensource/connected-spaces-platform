...
CSP_PUBLIC_TEST(CSPEngine, ConversationTests, ConversationComponentDeleteTest)
{
    ...
    // Delete the component to internally call DeleteConversation
    {
        ...
        AssetSystem->SetAssetDetailBlobChangedCallback(
            [ConversationId, &CallbackCalled](const csp::multiplayer::AssetDetailBlobChangedEventData& EventData)
            {
                EXPECT_EQ(EventData.ChangeType, csp::multiplayer::EAssetChangeType::Deleted);
                EXPECT_EQ(EventData.AssetCollectionId, ConversationId);
                CallbackCalled = true;
            });
...
CSP_PUBLIC_TEST(CSPEngine, ConversationTests, ConversationComponentEventTest)
{
    ...
    {
        csp::multiplayer::ConversationEventData RetrievedParams;
        bool CallbackCalled = false;

        const auto Callback = [&RetrievedParams, &CallbackCalled](const csp::multiplayer::ConversationEventData& Params)
        {
...
CSP_PUBLIC_TEST(CSPEngine, ConversationTests, ConversationComponentSecondClientEventDelayTest)
{
    ...
    {
        csp::multiplayer::ConversationEventData ReceivedParams;
        bool CallbackCalled = false;

        ConversationComponent->SetConversationUpdateCallback(
            [&CallbackCalled, &ReceivedParams, &ReceivedInfo](const csp::multiplayer::ConversationEventData& Params)
            {
