...
CSP_PUBLIC_TEST(CSPEngine, OfflineRealtimeEngineTests, ParentTest)
{
    ...
    Entity3->SetParentId(Entity1->GetParent()->GetId());
    ...
    if (Engine.GetRootHierarchyEntities()->Size() != 3)
    {
        FAIL();
    }

    EXPECT_EQ((*Engine.GetRootHierarchyEntities())[0]->GetId(), Entity1->GetId());
    EXPECT_EQ((*Engine.GetRootHierarchyEntities())[1]->GetId(), Entity2->GetId());
    EXPECT_EQ((*Engine.GetRootHierarchyEntities())[2]->GetId(), Entity3->GetId());
    ...
    if (Engine.GetRootHierarchyEntities()->Size() != 2)
    {
        FAIL();
    }
