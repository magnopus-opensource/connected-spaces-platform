...
import {
  ...
  jsArrayToCommonArray,
} from "../conversion_helpers.js";
...
export async function deleteSpace(
  spaceSystem,
  space,
  deleteFoundationResources = true
) {
  ...
  if (deleteFoundationResources) space.delete();
}
...
export async function createSpace(
  ...
  const result = await spaceSystem.createSpace(
    name,
    description,
    type,
    inviteUsers,
...
export async function updateSpace(
  ...
  const result = await spaceSystem.updateSpace(
    space.id,
    newName,
    newDescription,
    newType
...
export function createInviteUsers() {
  ...
  inviteUsers.inviteUserRoleInfos =
    Common.Array.ofcsp_systems_InviteUserRoleInfo_number(4);
