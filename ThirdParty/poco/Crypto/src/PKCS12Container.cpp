...
PKCS12Container::PKCS12Container(const std::string& path, const std::string& password): _pKey(0)
{
	...
	if (pFile)
	{
		...
		fclose(pFile);
		if (!pPKCS12)
			throw OpenSSLException("PKCS12Container(const string&, const string&)");
		load(pPKCS12, password);
	}
	else
	{
...
#else
	// Stdio has been disable in the OpenSSL config so this functionality is not supported
	throw OpenSSLException("PKCS12Container - Stdio disabled in OpenSSL config");
#endif
}
