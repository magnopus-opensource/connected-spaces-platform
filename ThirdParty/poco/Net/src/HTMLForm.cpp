...
void HTMLForm::write(std::ostream& ostr, bool countOnly)
{
	if (_encoding == ENCODING_URL)
		writeUrl(ostr);
	else
		writeMultipart(ostr, countOnly);
}
...
void HTMLForm::writeMultipart(std::ostream& ostr, bool countOnly)
{
	...
	for (const auto& part: _parts)
	{
		...
		if (countOnly)
		{
			HTMLFormCountingOutputStream* pCountingOutputStream = static_cast<HTMLFormCountingOutputStream*>(&ostr);
