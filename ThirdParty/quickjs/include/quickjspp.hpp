...
#ifdef CSP_ANDROID
// @note Android version less than r22 doesn't support std::filesystem
// this is a drop in alternative
...
// See class Value
static void csp_dump_obj(JSContext* ctx, FILE* f, JSValueConst val)
{
    const char* str;
...
static void csp_dump_error1(JSContext* ctx, JSValueConst exception_val)
{
    JSValue val;
    bool is_error;

    is_error = JS_IsError(ctx, exception_val);
    csp_dump_obj(ctx, stderr, exception_val);
    if (is_error)
    {
        val = JS_GetPropertyStr(ctx, exception_val, "stack");
        if (!JS_IsUndefined(val))
        {
            csp_dump_obj(ctx, stderr, val);
        }
        JS_FreeValue(ctx, val);
    }
}

inline void csp_dump_error(JSContext* ctx)
{
...
extern "C" JSModuleDef* js_get_aliased_module(JSContext* ctx, const char* base_cname, const char* cname1);
...
{
    static JSValue unwrap(JSContext* ctx, JSValueConst v) noexcept { return JS_DupValue(ctx, v); }
...
template <> struct js_traits<bool>
{
    ...
    static JSValue wrap(JSContext* ctx, bool i) noexcept { return JS_NewBool(ctx, i); }
};
...
template <> struct qjs::js_traits<float>
{
    /// @throws exception
...
template <> struct js_traits<std::string_view>
{
    ...
    static JSValue wrap(JSContext* ctx, std::string_view str) noexcept { return JS_NewStringLen(ctx, str.data(), str.size()); }
};
...
struct js_traits<std::string>
{
    ...
    static JSValue wrap(JSContext* ctx, const std::string& str) noexcept { return JS_NewStringLen(ctx, str.data(), str.size()); }
};

/** Conversion from const char * */
template <> struct js_traits<const char*>
{
    static JSValue wrap(JSContext* ctx, const char* str) noexcept { return JS_NewString(ctx, str); }

    static detail::js_string unwrap(JSContext* ctx, JSValueConst v) { return js_traits<std::string_view>::unwrap(ctx, v); }
};
...
template <typename... Ts> struct js_traits<std::variant<Ts...>>
{
    ...
    template <typename T> struct is_string
    {
        static constexpr bool value
            = std::is_same_v<T, const char*> || std::is_same_v<std::decay_t<T>, std::string> || std::is_same_v<std::decay_t<T>, std::string_view>;
    };
    ...
    template <typename U, typename... Us> static std::optional<std::variant<Ts...>> unwrapObj(JSContext* ctx, JSValueConst v, JSClassID class_id)
    {
    ...
    template <typename T> static bool isCompatible(JSContext* ctx, JSValueConst v) noexcept
    {
    ...
    static std::variant<Ts...> unwrap(JSContext* ctx, JSValueConst v)
    {
        ...
        case JS_TAG_OBJECT:
            ...
            JS_ThrowTypeError(ctx, "Expected type %s, got object with classid %d", QJSPP_TYPENAME(std::variant<Ts...>), JS_GetClassID(v, NULL));
            break;
...
namespace detail
{
    ...
    template <typename T, size_t I, size_t NArgs> struct unwrap_arg_impl
    {
        static auto unwrap(JSContext* ctx, int argc, JSValueConst* argv)
        {
            if (size_t(argc) <= I)
            {
                JS_ThrowTypeError(ctx, "Expected at least %lu arguments but received %d", (unsigned long)NArgs, argc);
                throw exception { ctx };
            }
    ...
    template <typename T, size_t I, size_t NArgs> struct unwrap_arg_impl<rest<T>, I, NArgs>
    {
    ...
    template <class Tuple, std::size_t... I> Tuple unwrap_args_impl(JSContext* ctx, int argc, JSValueConst* argv, std::index_sequence<I...>)
    {
     ...
     */
    template <typename... Args> std::tuple<std::decay_t<Args>...> unwrap_args(JSContext* ctx, int argc, JSValueConst* argv)
    {
     ...
     */
    template <typename R, typename... Args, typename Callable> JSValue wrap_call(JSContext* ctx, Callable&& f, int argc, JSValueConst* argv) noexcept
    {
        try
        {
            ...
            {
                return js_traits<std::decay_t<R>>::wrap(ctx, std::apply(std::forward<Callable>(f), unwrap_args<Args...>(ctx, argc, argv)));
            }
        }
        catch (exception)
        {
    ...
    JSValue wrap_this_call(JSContext* ctx, Callable&& f, JSValueConst this_value, int argc, JSValueConst* argv) noexcept
    {
        try
        {
            ...
            {
                std::apply(
                    std::forward<Callable>(f), std::tuple_cat(unwrap_args<FirstArg>(ctx, 1, &this_value), unwrap_args<Args...>(ctx, argc, argv)));
                return JS_NULL;
            }
            else
            {
                return js_traits<std::decay_t<R>>::wrap(ctx,
                    std::apply(std::forward<Callable>(f),
                        std::tuple_cat(unwrap_args<FirstArg>(ctx, 1, &this_value), unwrap_args<Args...>(ctx, argc, argv))));
            }
        }
        catch (exception)
        {
    ...
    template <class Tuple, std::size_t... I> void wrap_args_impl(JSContext* ctx, JSValue* argv, Tuple tuple, std::index_sequence<I...>)
    {
     ...
     */
    template <typename... Args> void wrap_args(JSContext* ctx, JSValue* argv, Args&&... args)
    {
 ...
 */
template <auto F, bool PassThis = false /* pass this as the first argument */> struct fwrapper
{
...
template <typename R, typename... Args, R (*F)(Args...), bool PassThis> struct js_traits<fwrapper<F, PassThis>>
{
    static JSValue wrap(JSContext* ctx, fwrapper<F, PassThis> fw) noexcept
    {
        return JS_NewCFunction(
            ctx,
            [](JSContext* ctx, JSValueConst this_value, int argc, JSValueConst* argv) noexcept -> JSValue
            {
...
template <typename R, class T, typename... Args, R (T::*F)(Args...), bool PassThis /*=ignored*/> struct js_traits<fwrapper<F, PassThis>>
{
    static JSValue wrap(JSContext* ctx, fwrapper<F, PassThis> fw) noexcept
    {
        return JS_NewCFunction(
...
template <typename R, class T, typename... Args, R (T::*F)(Args...) const, bool PassThis /*=ignored*/> struct js_traits<fwrapper<F, PassThis>>
{
    static JSValue wrap(JSContext* ctx, fwrapper<F, PassThis> fw) noexcept
    {
        return JS_NewCFunction(
...
template <class T, typename... Args> struct js_traits<ctor_wrapper<T, Args...>>
{
...
template <class T> struct js_traits<std::shared_ptr<T>>
{
     ...
     */
    template <typename D> static void registerDerivedClass(JSClassID derived_class_id, ptr_cast_fcn_t ptr_cast_fcn)
    {
            ...
            = [old_registerWithBase = std::move(old_registerWithBase)](JSClassID derived_class_id, derived_ptr_cast_fcn_t derived_ptr_cast_fcn)
        {
            ...
            registerDerivedClass<D>(derived_class_id,
                [derived_cast_fcn = std::move(derived_ptr_cast_fcn)](JSContext* ctx, JSValueConst v)
                { return std::shared_ptr<T>(derived_cast_fcn(ctx, v)); });
        };
    }

    template <typename B> static std::enable_if_t<std::is_same_v<B, T> || std::is_same_v<B, void>> ensureCanCastToBase() { }
    ...
    template <auto M> static void ensureCanCastToBase() { ensureCanCastToBase<detail::class_from_member_pointer_t<decltype(M)>>(); }
    ...
    static std::shared_ptr<T> unwrap(JSContext* ctx, JSValueConst v)
    {
        ...
        auto obj_class_id = JS_GetClassID(v, NULL);
        ...
        else
        {
            ...
            JS_ThrowTypeError(ctx, "Expected type %s, got object with classid %d", QJSPP_TYPENAME(T), obj_class_id);
            throw exception { ctx };
        }
        if (!ptr)
        {
...
namespace detail
{
    ...
    struct function
    {
        ...
        template <typename Functor> static function* create(JSRuntime* rt, Functor&& f)
        {
...
template <> struct js_traits<detail::function>
{
    ...
    static void register_class(JSContext* ctx, const char* name)
    {
        ...
        JSClassDef def { name,
            ...
            nullptr, // mark
            // call
            [](JSContext* ctx, JSValueConst func_obj, JSValueConst this_val, int argc, JSValueConst* argv, int flags) -> JSValue
            {
...
template <> struct js_property_traits<const char*>
{
    ...
    static JSValue get_property(JSContext* ctx, JSValue this_obj, const char* name) noexcept { return JS_GetPropertyStr(ctx, this_obj, name); }
};
...
template <> struct js_property_traits<uint32_t>
{
    ...
    static JSValue get_property(JSContext* ctx, JSValue this_obj, uint32_t idx) noexcept { return JS_GetPropertyUint32(ctx, this_obj, idx); }
};
...
namespace detail
{
    template <typename Key> struct property_proxy
    {
        ...
        template <typename T> T as() const { return unwrap_free<T>(ctx, js_property_traits<Key>::get_property(ctx, this_obj, key)); }
        ...
        {
            js_property_traits<Key>::set_property(ctx, this_obj, key, js_traits<std::decay_t<T>>::wrap(ctx, std::forward<T>(value)));
            return *this;
        }

        template <typename Key2> property_proxy<Key2> operator[](Key2 key2) const { return { ctx, as<JSValue>(), std::move(key2) }; }
 ...
 * A JSValue can be released by either JSValue x = std::move(value); or JSValue x = value.release(), then the Value becomes invalid and FreeValue
 * won't be called Can be converted to C++ type, for example: auto string = value.as<std::string>(); qjs::exception would be thrown on error
...
public:
        ...
        : ctx(ctx)
    {
        ...
        {
            // *************************
            // Oly - Remove exception so we can parse any errors
            csp_dump_error(ctx);
            // throw exception {ctx};
            // *************************
        }
     ...
     * */
    template <typename T> auto as() const { return js_traits<std::decay_t<T>>::unwrap(ctx, v); }
    ...
    {
        (*this)[name] = js_traits<decltype(std::function { std::forward<Function>(f) })>::wrap(ctx, std::forward<Function>(f));
        return *this;
    }

    // add<&f>("f");
    // add<&T::f>("f");
    template <auto F> std::enable_if_t<!std::is_member_object_pointer_v<decltype(F)>, Value&> add(const char* name)
    {
    ...
    template <auto FGet, auto FSet> Value& add_getter_setter(const char* name)
    {
        ...
        int ret = JS_DefinePropertyGetSet(ctx, v, prop, js_traits<fgetter>::wrap(ctx, fgetter { name }),
            js_traits<fsetter>::wrap(ctx, fsetter { name }), JS_PROP_CONFIGURABLE | JS_PROP_WRITABLE | JS_PROP_ENUMERABLE);
    ...
    template <auto FGet> Value& add_getter(const char* name)
    {
        ...
        int ret = JS_DefinePropertyGetSet(
            ctx, v, prop, js_traits<fgetter>::wrap(ctx, fgetter { name }), JS_UNDEFINED, JS_PROP_CONFIGURABLE | JS_PROP_ENUMERABLE);
        JS_FreeAtom(ctx, prop);
        if (ret < 0)
            throw exception { ctx };
        return *this;
    ...
    // add<&T::member>("member");
    template <auto M> std::enable_if_t<std::is_member_object_pointer_v<decltype(M)>, Value&> add(const char* name)
    {
    ...
    std::string toJSON(const Value& replacer = JS_UNDEFINED, const Value& space = JS_UNDEFINED)
    {
        assert(ctx);
...
private:
    static void promise_unhandled_rejection_tracker(JSContext* ctx, JSValueConst promise, JSValueConst reason, JS_BOOL is_handled, void* opaque);
...
namespace detail
{
    ...
    // @note Android version less than r22 doesn't support std::filesystem
    inline std::optional<std::string> readFile(ghc::filesystem::path const& filepath)
    {
    ...
    inline std::string toUri(std::string_view filename)
    {
        ...
        auto fpath = ghc::filesystem::path(fname);
        if (!fpath.is_absolute())
        {
            fpath = "." / fpath;
        }
        fpath = ghc::filesystem::weakly_canonical(fpath);
...
public:
    ...
    class Module
    {
        ...
        // **************************
        // WillC - Changed to std::string so non string literals can be used
    ...
    public:
        Module(JSContext* ctx, const char* name)
            ...
            , name(name)
        {
            m = JS_NewCModule(ctx, name,
                [](JSContext* ctx, JSModuleDef* m) noexcept
                {
                    auto& context = Context::get(ctx);
                    auto it = std::find_if(context.modules.begin(), context.modules.end(), [m](const Module& module) { return module.m == m; });
                    if (it == context.modules.end())
                        return -1;
                    for (const auto& e : it->exports)
                    {
                        if (JS_SetModuleExport(ctx, m, e.first.c_str(), JS_DupValue(ctx, e.second.v)) != 0)
                            return -1;
                    }
                    return 0;
                });
        ...
        template <typename T> Module& add(const char* name, T&& value) { return add(name, js_traits<T>::wrap(ctx, std::forward<T>(value))); }
         ...
         */
        template <auto F> Module& function(const char* name) { return add(name, qjs::fwrapper<F> { name }); }
    ...
    private:
        ...
        public:
            ...
            template <auto FGet, auto FSet = nullptr> class_registrar& property(const char* name)
            {
             ...
             */
            template <typename... Args> class_registrar& constructor(const char* name = nullptr)
            {
                if (!name)
                    name = this->name;
            ...
            ~class_registrar() { context.registerClass<T>(name, std::move(prototype)); }
        };
    ...
    public:
         ...
         */
        template <class T> class_registrar<T> class_(const char* name) { return class_registrar<T> { name, *this, qjs::Context::get(ctx) }; }
    };
...
public:
    ...
    struct ModuleData
    {
        std::optional<std::string> source, url, alias;
        ModuleData()
            : source(std::nullopt)
            , url(std::nullopt)
            , alias(std::nullopt)
        ...
        ModuleData(std::optional<std::string> url, std::optional<std::string> source, std::optional<std::string> alias)
            : source(std::move(source))
            , url(std::move(url))
    ...
    std::function<ModuleData(std::string_view)> moduleLoader = [](std::string_view filename) -> ModuleData {
        return ModuleData { detail::toUri(filename), detail::readFile(filename), std::nullopt };
    };
    ...
    template <typename T> Value newValue(T&& val) { return Value { ctx, std::forward<T>(val) }; }
     ...
     */
    template <class T> void registerClass(const char* name, JSValue proto = JS_NULL)
    {
...
{
    static Value unwrap(JSContext* ctx, JSValueConst v) { return Value { ctx, JS_DupValue(ctx, v) }; }
 ...
 * @tparam Args argument types
 */
template <typename R, typename... Args> struct js_traits<std::function<R(Args...)>, int>
{
     ...
     */
    template <typename Functor> static JSValue wrap(JSContext* ctx, Functor&& functor) noexcept
    {
...
namespace detail
{
    ...
    template <typename T> struct is_callable<T, std::enable_if_t<std::is_same_v<decltype(void(&T::operator())), void>>> : std::true_type
    {
    };
...
template <typename Function> struct js_traits<Function, std::enable_if_t<detail::is_callable_v<Function>>>
{
    static auto unwrap(JSContext* ctx, JSValueConst fun_obj)
    {
        return js_traits<decltype(std::function { std::declval<Function>() }), int>::unwrap(ctx, fun_obj);
    }

    template <typename Functor> static JSValue wrap(JSContext* ctx, Functor&& functor)
    {
        return js_traits<decltype(std::function { std::declval<Function>() }), int>::wrap(ctx, std::forward<Functor>(functor));
    }
};
...
template <typename U, typename V> struct js_traits<std::pair<U, V>>
{
    ...
    static std::pair<U, V> unwrap(JSContext* ctx, JSValueConst jsarr)
    {
        ...
        return std::pair<U, V> { static_cast<U>(jsarray[uint32_t(0)]), static_cast<V>(jsarray[uint32_t(1)]) };
    }
};
...
namespace detail
{
    template <typename Key> property_proxy<Key>::operator Value() const { return as<Value>(); }
} // namespace detail
...
inline void Runtime::promise_unhandled_rejection_tracker(JSContext* ctx, JSValueConst promise, JSValueConst reason, JS_BOOL is_handled, void* opaque)
{
    auto& context = Context::get(ctx);
...
inline JSModuleDef* Runtime::module_loader(JSContext* ctx, const char* module_name, void* opaque)
{
    ...
    try
    {
        ...
        if (data.alias)
        {
            return js_get_aliased_module(ctx, "", data.alias->c_str());
        }
