...
public:
	template <typename... Args> js_string(Args&&... args) : Base(std::forward<Args>(args)...), ctx(nullptr)
	{
	}
...
template <class T> struct js_traits<T*, std::enable_if_t<std::is_class_v<T>>>
{
	static JSValue wrap(JSContext* ctx, T* ptr)
	{
		...
		auto pptr = new std::shared_ptr<T>(ptr,
										   [](T*)
										   {
										   });
		JS_SetOpaque(jsobj, pptr);
...
public:
	...
	template <auto FGet, auto FSet> Value& add_getter_setter(const char* name)
	{
		...
		int ret		  = JS_DefinePropertyGetSet(ctx,
											v,
											prop,
		...
		Module(JSContext* ctx, const char* name) : ctx(ctx), name(name)
		{
			m = JS_NewCModule(ctx,
							  ...
							  [](JSContext* ctx, JSModuleDef* m) noexcept
							  {
								  ...
								  auto it		= std::find_if(context.modules.begin(),
														   context.modules.end(),
														   [m](const Module& module)
														   {
															   return module.m == m;
														   });
	...
	private:
		...
		public:
			explicit class_registrar(const char* name, qjs::Context::Module& module, qjs::Context& context)
				: name(name), prototype(context.newObject()), module(module), context(context)
			{
			}
...
public:
	...
	struct ModuleData
	{
		...
		ModuleData() : source(std::nullopt), url(std::nullopt), alias(std::nullopt)
		{
		}
		ModuleData(std::optional<std::string> source) : source(std::move(source)), url(std::nullopt), alias(std::nullopt)
		{
		}
		ModuleData(std::optional<std::string> url, std::optional<std::string> source, std::optional<std::string> alias)
			: source(std::move(source)), url(std::move(url)), alias(std::move(alias))
		{
		}
	};
