...
#define BN_prime_checks_for_size(b) \
	((b) >= 3747 ? 3 : (b) >= 1345 ? 4 : (b) >= 476 ? 5 : (b) >= 400 ? 6 : (b) >= 347 ? 7 : (b) >= 308 ? 8 : (b) >= 55 ? 27 : /* b >= 6 */ 34)
...
#define BN_mod(rem, m, d, ctx) BN_div(NULL, (rem), (m), (d), (ctx))
	...
	/* Deprecated versions */
	DEPRECATEDIN_0_9_8(BIGNUM *BN_generate_prime(BIGNUM *ret,
												 int bits,
												 int safe,
												 const BIGNUM *add,
												 const BIGNUM *rem,
...
#define BN_BLINDING_NO_RECREATE 0x00000002
	...
	BN_BLINDING *
		BN_BLINDING_create_param(BN_BLINDING *b,
								 const BIGNUM *e,
								 BIGNUM *m,
								 BN_CTX *ctx,
