...
#define PKCS12_OK	 1
	...
	unsigned char *PKCS12_pbe_crypt(const X509_ALGOR *algor,
									...
									int en_de);
	void *
		PKCS12_item_decrypt_d2i(const X509_ALGOR *algor, const ASN1_ITEM *it, const char *pass, int passlen, const ASN1_OCTET_STRING *oct, int zbuf);
	...
	int PKCS12_key_gen_uni(unsigned char *pass,
						   ...
						   unsigned char *out,
						   const EVP_MD *md_type);
	int PKCS12_key_gen_utf8(const char *pass,
							int passlen,
							unsigned char *salt,
							int saltlen,
							int id,
	...
	PKCS12 *PKCS12_create(const char *pass,
						  const char *name,
						  EVP_PKEY *pkey,
						  X509 *cert,
						  STACK_OF(X509) * ca,
