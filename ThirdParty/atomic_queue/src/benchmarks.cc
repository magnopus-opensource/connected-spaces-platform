...
template <class T> using Type = std::common_type<T>; // Similar to boost::type<>.
...
template <class T> inline double to_seconds(T tsc)
{
...
template <class Queue> struct BoostSpScAdapter : Queue
{
...
template <class Queue> struct BoostQueueAdapter : BoostSpScAdapter<Queue>
{
	using T = typename Queue::value_type;
...
template <class Queue> struct XeniumQueueAdapter : Queue
{
	using T = typename Queue::value_type;
...
template <class T> struct region_guard_traits
{
	struct region_guard
	{
...
template <class T, class... Policies> struct region_guard_traits<xenium::michael_scott_queue<T, Policies...>>
{
	using region_guard = typename xenium::michael_scott_queue<T, Policies...>::region_guard;
};
template <class T, class... Policies> struct region_guard_traits<xenium::ramalhete_queue<T, Policies...>>
{
	using region_guard = typename xenium::ramalhete_queue<T, Policies...>::region_guard;
};

template <class T> using region_guard_t = typename region_guard_traits<T>::region_guard;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

template <class Queue, size_t Capacity> struct TbbAdapter : RetryDecorator<Queue>
{
	TbbAdapter()
	{
...
template <class Queue, size_t Capacity> struct CapacityToConstructor : Queue
{
...
template <class T> struct ConstructorAdapter : T
{
	using T::T;
};
...
template <unsigned SIZE, bool SPSC, bool MINIMIZE_CONTENTION, bool MAXIMIZE_THROUGHPUT> struct QueueTypes
{
	using T = unsigned;
	...
	using AtomicQueueB
		= Type<RetryDecorator<CapacityToConstructor<atomic_queue::AtomicQueueB<T, Allocator, T {}, MAXIMIZE_THROUGHPUT, false, SPSC>, SIZE>>>;
	...
	using AtomicQueueB2
		= Type<RetryDecorator<CapacityToConstructor<atomic_queue::AtomicQueueB2<T, Allocator, MAXIMIZE_THROUGHPUT, false, SPSC>, SIZE>>>;
...
template <class Queue> void throughput_producer(unsigned N, Queue* queue, std::atomic<cycles_t>* t0, Barrier* barrier)
{
	barrier->wait();
							  ...
							  sum_t* consumer_sums)
{
	...
	if (alternative_placement)
	{
		for (unsigned i = 0; i < thread_count; ++i)
		{
			...
			if (i != thread_count - 1)
			{ // This thread is the last consumer.
				set_default_thread_affinity(hw_thread_ids[cpu_idx++]);
			...
			for (unsigned run = RUNS; run--;)
			{
				...
				for (unsigned i = 0; i < threads; ++i)
				{
					...
					if (consumer_sum_frac < .1)
						std::fprintf(stderr,
									 "%s: producers: %u: consumer %u received too few messages: %.2lf%% of expected.\n",
									 name,
									 threads,
									 i,
									 consumer_sum_frac);
				...
				if (auto diff = total_sum - expected_sum * threads)
					std::fprintf(stderr,
								 "%s: wrong checksum error: producers: %u, expected_sum: %'lld, diff: %'lld.\n",
								 name,
								 threads,
								 expected_sum * threads,
								 diff);
			}
...
template <class Queue>
void run_throughput_mpmc_benchmark(char const* name,
								   HugePages& hp,
								   std::vector<unsigned> const& hw_thread_ids,
								   Type<Queue>,
								   unsigned thread_count_min = 1)
{
...
template <class... Args>
void run_throughput_spsc_benchmark(char const* name,
								   HugePages& hp,
								   std::vector<unsigned> const& hw_thread_ids,
...
template <class Queue> void run_throughput_spsc_benchmark(char const* name, HugePages& hp, std::vector<unsigned> const& hw_thread_ids, Type<Queue>)
{
...
void run_throughput_benchmarks(HugePages& hp, std::vector<CpuTopologyInfo> const& cpu_topology)
{
	...
	run_throughput_spsc_benchmark("boost::lockfree::spsc_queue",
								  hp,
								  hw_thread_ids,
								  Type<BoostSpScAdapter<boost::lockfree::spsc_queue<unsigned, boost::lockfree::capacity<SIZE>>>> {});
	run_throughput_mpmc_benchmark("boost::lockfree::queue",
								  hp,
								  hw_thread_ids,
	...
	run_throughput_mpmc_benchmark("tbb::concurrent_bounded_queue",
								  hp,
								  hw_thread_ids,
								  Type<TbbAdapter<tbb::concurrent_bounded_queue<unsigned>, SIZE>> {});

	run_throughput_mpmc_benchmark("xenium::michael_scott_queue",
								  hp,
								  hw_thread_ids,
								  Type<XeniumQueueAdapter<xenium::michael_scott_queue<unsigned, xenium::policy::reclaimer<Reclaimer>>>> {});
	run_throughput_mpmc_benchmark("xenium::ramalhete_queue",
								  hp,
								  hw_thread_ids,
								  Type<XeniumQueueAdapter<xenium::ramalhete_queue<unsigned, xenium::policy::reclaimer<Reclaimer>>>> {});
	run_throughput_mpmc_benchmark("xenium::vyukov_bounded_queue",
								  hp,
								  hw_thread_ids,
...
template <class Queue> void ping_pong_thread_impl(Queue* q1, Queue* q2, unsigned N, cycles_t* time, std::false_type /*sender*/)
{
...
template <class Queue> void ping_pong_thread_impl(Queue* q1, Queue* q2, unsigned N, cycles_t* time, std::true_type /*sender*/)
{
	cycles_t t0 = __builtin_ia32_rdtsc();
	region_guard_t<Queue> guard;
...
template <class Queue> inline void ping_pong_thread_receiver(Barrier* barrier, Queue* q1, Queue* q2, unsigned N, cycles_t* time)
{
	barrier->wait();
...
template <class Queue> inline void ping_pong_thread_sender(Barrier* barrier, Queue* q1, Queue* q2, unsigned N, cycles_t* time)
{
	barrier->release(1);
...
template <class Queue> inline std::array<cycles_t, 2> ping_pong_benchmark(unsigned N, HugePages& hp, unsigned const (&cpus)[2])
{
...
template <class Queue> void run_ping_pong_benchmark(char const* name, HugePages& hp, std::vector<unsigned> const& hw_thread_ids)
{
...
void run_ping_pong_benchmarks(HugePages& hp, std::vector<CpuTopologyInfo> const& cpu_topology)
{
	...
	run_ping_pong_benchmark<BoostSpScAdapter<boost::lockfree::spsc_queue<unsigned, boost::lockfree::capacity<SIZE>>>>("boost::lockfree::spsc_queue",
																													  hp,
																													  hw_thread_ids);
	run_ping_pong_benchmark<BoostQueueAdapter<boost::lockfree::queue<unsigned, BoostAllocator, boost::lockfree::capacity<SIZE>>>>(
	...
	// run_ping_pong_benchmark<RetryDecorator<AtomicQueueMutex<unsigned, SIZE, tbb::speculative_spin_mutex>>>("tbb::speculative_spin_mutex", hp,
	// hw_thread_ids);
	...
	run_ping_pong_benchmark<XeniumQueueAdapter<xenium::michael_scott_queue<unsigned, xenium::policy::reclaimer<Reclaimer>>>>(
		"xenium::michael_scott_queue",
		hp,
		hw_thread_ids);
	run_ping_pong_benchmark<XeniumQueueAdapter<xenium::ramalhete_queue<unsigned, xenium::policy::reclaimer<Reclaimer>>>>("xenium::ramalhete_queue",
																														 hp,
																														 hw_thread_ids);
	run_ping_pong_benchmark<RetryDecorator<CapacityToConstructor<xenium::vyukov_bounded_queue<unsigned>, SIZE>>>("xenium::vyukov_bounded_queue",
																												 hp,
																												 hw_thread_ids);
