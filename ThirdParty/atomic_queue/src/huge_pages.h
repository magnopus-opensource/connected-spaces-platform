...
public:
	...
	HugePages(HugePages&& b) noexcept : cur_(b.cur_), end_(b.end_), beg_(b.beg_)
	{
		b.beg_ = 0;
	}
	...
	template <class T, class... Args> std::unique_ptr<T, Deleter> create_unique_ptr(Args&&... args)
	{
		return std::unique_ptr<T, Deleter> {new (this->allocate(sizeof(T))) T {std::forward<Args>(args)...}, Deleter {this}};
	}

	template <class T, class... Args> std::unique_ptr<T, Deleter> create_unique_ptr(NoContext, Args&&... args)
	{
		return std::unique_ptr<T, Deleter> {new (this->allocate(sizeof(T))) T {std::forward<Args>(args)...}, Deleter {this}};
	}
...
template <class T> struct HugePageAllocator : HugePageAllocatorBase
{
