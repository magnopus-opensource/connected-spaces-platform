...
struct NoContext
{
	template <class... Args> constexpr NoContext(Args&&...) noexcept
	{
	}
};
...
struct NoToken
{
	template <class... Args> constexpr NoToken(Args&&...) noexcept
	{
	}

	template <class Queue, class T> static void push(Queue& q, T&& element) noexcept
	{
		q.push(std::forward<T>(element));
	}

	template <class Queue> static auto pop(Queue& q) noexcept
	{
		return q.pop();
	}
};
