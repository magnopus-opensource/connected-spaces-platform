...
template <class M> struct ScopedLockType
{
	using type = typename M::scoped_lock;
};

template <> struct ScopedLockType<std::mutex>
{
...
template <class T, class Mutex, unsigned SIZE, bool MINIMIZE_CONTENTION> class AtomicQueueMutexT
{
...
public:
	...
	template <class U> bool try_push(U&& element) noexcept
	{
