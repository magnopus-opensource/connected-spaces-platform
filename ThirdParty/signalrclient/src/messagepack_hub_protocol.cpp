...
namespace signalr
{
// replaces use of msgpack::sbuffer when packing so that we can write directly to the string instead of writing to another buffer and copying to a
// string later
...
signalr::value createValue(const msgpack::object& v)
{
	switch (v.type)
	{
		...
		case msgpack::type::object_type::POSITIVE_INTEGER:
			return signalr::value(v.via.u64);
		case msgpack::type::object_type::NEGATIVE_INTEGER:
			return signalr::value(v.via.i64);
		...
		{
			auto ptr = v.via.bin.ptr;
			auto len = v.via.bin.size;
			return signalr::value((uint8_t*) ptr, len);
		}
		case msgpack::type::object_type::MAP:
		{
			if (v.via.map.size == 0)
			{
				return signalr::value_type::null;
			}

			if (v.via.map.ptr->key.type == msgpack::type::object_type::STR)
			{
				// Map uses string keys
				std::map<std::string, signalr::value> map;
				for (size_t i = 0; i < v.via.map.size; ++i)
				{
					auto* pair		= v.via.map.ptr + i;
					std::string key = pair->key.as<std::string>();
			...
			{
				// Assume integer type key
				std::map<uint64_t, signalr::value> map;
				for (size_t i = 0; i < v.via.map.size; ++i)
				{
					auto* pair	 = v.via.map.ptr + i;
					uint64_t key = pair->key.via.i64;
					map.insert({key, createValue((v.via.map.ptr + i)->val)});
				}
				return signalr::value(std::move(map));
			}
...
void pack_messagepack(const signalr::value& v, msgpack::packer<string_wrapper>& packer)
{
	switch (v.type())
	{
		...
		{
			auto int_val = v.as_integer();
			packer.pack_int64(int_val);
			return;
		}
		case signalr::value_type::uinteger:
		{
			auto int_val = v.as_uinteger();
			packer.pack_uint64(int_val);
			return;
		}
		...
		case signalr::value_type::raw:
		{
			size_t len;
			auto ptr = v.as_raw(len);
			packer.pack_bin(static_cast<uint32_t>(len));
			packer.pack_bin_body((char*) ptr, static_cast<uint32_t>(len));
			return;
		}
		case signalr::value_type::string_map:
		{
			const auto& obj = v.as_string_map();
		...
		{
			const auto& obj = v.as_uint_map();
			packer.pack_map((uint32_t) obj.size());
			for (auto& val : obj)
			{
				auto int_val = val.first;
				packer.pack_uint64(int_val);
				pack_messagepack(val.second, packer);
			}
			return;
...
std::vector<std::unique_ptr<hub_message>> messagepack_hub_protocol::parse_messages(const std::string& message) const
{
	...
	while (binary_message_parser::try_parse_message(reinterpret_cast<const unsigned char*>(remaining_message),
