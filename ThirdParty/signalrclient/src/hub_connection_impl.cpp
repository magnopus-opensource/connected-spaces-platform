...
namespace
{
static std::function<void(const char*, const signalr::value&)>
	create_hub_invocation_callback(const logger& logger,
...
void hub_connection_impl::initialize()
{
	...
	m_connection->set_message_received(
		[weak_hub_connection](std::string&& message)
		{
			auto connection = weak_hub_connection.lock();
			if (connection)
			{
	...
	m_connection->set_disconnected(
		[weak_hub_connection](std::exception_ptr exception)
		{
			auto connection = weak_hub_connection.lock();
			if (connection)
			{
				// start may be waiting on the handshake response so we complete it here, this no-ops if already set
				// Don't try to make an exception_ptr here as this throws! WE DON'T KNOW WHY. - MK
				...
				connection->m_handshakeTask->set();
...
void hub_connection_impl::start(std::function<void(std::exception_ptr)> callback) noexcept
{
	...
	{
		callback(std::make_exception_ptr(signalr_exception("the connection can only be started if it is in the disconnected state")));
		return;
	}
	...
	m_connection->start(
		[weak_connection, callback](std::exception_ptr start_exception)
		{
			auto connection = weak_connection.lock();
			if (!connection)
			{
			...
			if (start_exception)
			{
				connection->m_connection->stop(
					[callback, weak_connection](std::exception_ptr ex)
					{
						try
						{
							auto connection = weak_connection.lock();
			...
			connection->m_connection->send(handshake_request,
										   connection->m_protocol->transfer_format(),
										   [weak_connection, callback](std::exception_ptr exception)
										   {
											   auto connection = weak_connection.lock();
											   if (!connection)
											   {
											   ...
											   catch (...)
											   {
												   ...
												   connection->m_connection->stop(
													   [callback, handshake_exception](std::exception_ptr)
													   {
											   ...
											   {
												   connection->m_connection->stop(
													   [callback, exception](std::exception_ptr)
													   {
														   callback(exception);
													   },
													   exception);
...
void hub_connection_impl::process_message(std::string&& response)
{
	try
	{
		if (!m_handshakeReceived)
		{
			...
			auto& obj  = handshake.as_string_map();
			...
			if (found != obj.end())
			{
				...
				{
					m_logger.log(trace_level::error, std::string("handshake error: ").append(error));
				}
			...
			else
			{
				...
				if (found != obj.end())
				{
					m_handshakeTask->set(std::make_exception_ptr(
						signalr_exception(std::string("Received unexpected message while waiting for the handshake response."))));
				}
	...
	catch (const std::exception& e)
	{
		if (m_logger.is_enabled(trace_level::error))
		{
			m_logger.log(trace_level::error,
						 std::string("error occured when parsing response: ").append(e.what()).append(". response: ").append(response));
		}
...
void hub_connection_impl::invoke(const std::string& method_name,
								 ...
								 std::function<void(const signalr::value&, std::exception_ptr)> callback) noexcept
{
	...
	const auto& callback_id = m_callback_manager.register_callback(create_hub_invocation_callback(
		m_logger,
		[callback](const signalr::value& result)
		{
...
void hub_connection_impl::send(const std::string& method_name,
							   const signalr::value& arguments,
							   std::function<void(std::exception_ptr)> callback) noexcept
{
	if (!arguments.is_array())
	{
...
void hub_connection_impl::invoke_hub_method(const std::string& method_name,
											const signalr::value& arguments,
											const std::string& callback_id,
											std::function<void()> set_completion,
											std::function<void(const std::exception_ptr)> set_exception) noexcept
{
	try
	{
		...
		m_connection->send(message,
						   m_protocol->transfer_format(),
						   [set_completion, set_exception, weak_hub_connection, callback_id](std::exception_ptr exception)
						   {
							   if (exception)
							   {
...
void hub_connection_impl::start_keepalive()
{
	...
	auto send_ping = [](std::shared_ptr<hub_connection_impl> connection)
	{
		...
		try
		{
			...
			connection->m_connection->send(message,
										   connection->m_protocol->transfer_format(),
										   [weak_connection](std::exception_ptr exception)
										   {
											   auto connection = weak_connection.lock();
...
namespace
{
static std::function<void(const char* error, const signalr::value&)>
	create_hub_invocation_callback(const logger& logger,
								   const std::function<void(const signalr::value&)>& set_result,
