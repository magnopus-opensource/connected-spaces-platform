...
namespace signalr
{
    namespace details
    {
        namespace
        {
            ...
            struct inner_parse_out
            {
                    ...
                    if (*p == '/' && p[1] == '/')
                    {
                        ...
                        if (authority_begin != authority_end)
                        {
                            ...
                            if (*port_begin == ':')
                            {
                                ...
                                port =
                                    utility::scan_string<int>(std::string(port_begin, authority_end));
                            }
                            else
                            {
                                // no port
                ...
                void write_to(uri_components& components)
                {
                    if (scheme_begin)
                    {
                        ...
                        utility::inplace_tolower(components.m_scheme);
                    }
                    else
                    {
                    ...
                    if (host_begin)
                    {
                        ...
                        utility::inplace_tolower(components.m_host);
                    }
                    else
                    {
            ...
            template<class F>
            std::string encode_impl(const std::string& raw, F should_encode)
            {
            ...
            {
                const ::std::string dotSegment = ".";
                const ::std::string dotDotSegment = "..";

                if (builder.path().find('.') == std::string::npos) return;
                ...
                if (result.empty())
                {
                    builder.set_path(std::string());
                    return;
                }
                std::string path = result.front().get();
                for (size_t i = 1; i != result.size(); ++i)
                {
                    path += '/';
                    path += result[i].get();
                }
                if (segments.back() == dotDotSegment || segments.back() == dotSegment || builder.path().back() == '/')
                {
                    path += '/';
                }
        ...
        std::string uri_components::join()
        {
            // canonicalize components first

            // convert scheme to lowercase
            utility::inplace_tolower(m_scheme);
            // convert host to lowercase
            utility::inplace_tolower(m_host);
    ...
    uri::uri(const details::uri_components& components) : m_components(components)
    {
        ...
        {
            throw uri_exception("provided uri is invalid: " + m_uri);
        }
    }

    uri::uri(const std::string& uri_string) : uri(uri_string.c_str()) {}
    ...
    std::string uri::encode_query_impl(const std::string& raw)
    {
    ...
    /// </summary>
    std::string uri::encode_data_string(const std::string& data)
    {
        return details::encode_impl(data, [](int ch) -> bool { return !details::is_unreserved(ch); });
    }

    std::string uri::encode_uri(const std::string& raw, uri::components::component component)
    {
    ...
    std::string uri::decode(const std::string& encoded) { return decode_template(encoded); }

    std::vector<std::string> uri::split_path(const std::string& path)
    {
    ...
    std::map<std::string, std::string> uri::split_query(const std::string& query)
    {
    ...
    std::string uri::resolve_uri(const std::string& relativeUri) const
    {
        ...
        if (relativeUri[0] == '/') // starts with '/'
        {
            ...
            {
                return this->scheme() + ':' + relativeUri;
            }
