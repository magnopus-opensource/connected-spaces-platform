...
public:
	...
	OLY_NO_EXPORT virtual void
		SetUploadContent(oly_web::WebClient* InWebClient, oly_web::HttpPayload* InPayload, const oly_systems::Asset& InAsset) const = 0;
...
protected:
	AssetsResult() = delete;
	AssetsResult(void*) {};
	AssetsResult(oly_services::EResultCode ResCode, uint16_t HttpResCode) : oly_services::ResultBase(ResCode, HttpResCode) {};
...
private:
	...
	UriResult(oly_services::EResultCode ResCode, uint16_t HttpResCode) : oly_services::ResultBase(ResCode, HttpResCode) {};
	void OnResponse(const oly_services::ApiResponseBase* ApiResponse) override;
