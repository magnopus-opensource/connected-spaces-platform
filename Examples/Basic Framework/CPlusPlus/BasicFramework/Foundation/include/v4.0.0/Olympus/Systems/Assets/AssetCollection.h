...
public:
    ...
    /// @brief Retrieves a mutable version of the asset collection's metadata. To be used when it is necessary to mutate the asset collection's metadata.
...
private:
	...
	AssetCollectionsResult(oly_services::EResultCode ResCode, uint16_t HttpResCode) : oly_services::ResultBase(ResCode, HttpResCode) {};
