...
public class HelloWorld : MonoBehaviour
{
    ...
    private async void SignInAsync(string email, string password)
    {
        ...
        //Replace with your own credentials until input fields are working
        await LoginAsync("example", "example");
    ...
    private async Task LoginAsync(string email, string password)
    {
        ...
        using var loginStateResult = loginResult.GetLoginState();
        if (loginStateResult.State == CspSystems.ELoginState.LoggedIn)
        {
            userId = loginStateResult.UserId;
            Debug.Log($"Logged in with Email {email}.");
        }
        else
        {
            Debug.LogError($"Failed to log in, login state: {loginStateResult.State}");
        }
    }
    ...
    private async void CreateSpaceAsync(string spaceName)
    {
        ...
        using var cspMetadata = ToCSPMap(metadata);
        ...
        using CspSystems.SpaceResult spaceResult = await spaceSystem.CreateSpace(spaceName, string.Empty,
            CspSystems.SpaceAttributes.Public, null, cspMetadata, null);
    ...
    private async void EnterSpaceAsync(string spaceId, bool useCreatedSpace)
    {
        ...
        Debug.Log($"getting space {targetSpaceId}");
    ...
    private async Task SearchSpacesAsync()
    {
        ...
        using CspCommon.Array<CspSystems.Space> spaces = response.GetSpaces();
        CspSystems.Space[] unitySpaces = ToUnityArray(spaces);

        Debug.Log("Got " + unitySpaces.Length + " Spaces");

        foreach (var space in unitySpaces)
        {
            space.Dispose();
        }
    }
    ...
    private async Task EnterSpaceAsync(CspSystems.Space space)
    {
        ...
        //TODO: dispose of this on shutdown or disconnect
    ...
    private async Task InitializeConnection(string spaceId)
    {
        ...
        if (connectResult == ErrorCode.None)
        {
            Debug.Log($"Connected to multiplayer");
            
            var initialiseResult = await connection.InitialiseConnection();

            if (initialiseResult == ErrorCode.None)
            {
                Debug.Log($"Connection initialized");
            }
            else
            {
                Debug.LogError($"Connection initialization failed, error code: {initialiseResult}");
            }
           
        }
        else
        {
            Debug.LogError($"Failed to connect to multiplayer, error code: {connectResult}");
        }
    }
    ...
    {
        using var avatarSpaceTransform = new SpaceTransform(CspCommon.Vector3.Zero(), CspCommon.Vector4.Identity(), CspCommon.Vector3.One());
        SpaceEntity entity = await entitySystem.CreateAvatar($"user-{userId}",avatarSpaceTransform, AvatarState.Idle, userId, AvatarPlayMode.Default);
    ...
    private void CreateRemoteAvatarGameObject(SpaceEntity spaceEntity)
    {
        ...
        using var cspPosition = spaceEntity.GetPosition();
        using var cspRotation = spaceEntity.GetRotation();
        using var cspScale = spaceEntity.GetScale();
        
        Vector3 pos = cspPosition.ToUnityVector().ToUnityPositionFromGLTF();
        Quaternion rot = cspRotation.ToUnityQuaternion().ToUnityRotationFromGLTF();
        Vector3 scale = cspScale.ToUnityVector().ToUnityScaleFromGLTF();
