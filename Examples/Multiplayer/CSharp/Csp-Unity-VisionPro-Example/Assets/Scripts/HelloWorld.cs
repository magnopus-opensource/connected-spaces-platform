...
public class HelloWorld : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        accountUI.SignInButtonClicked += SignInAsync;
        accountUI.SignUpButtonClicked += SignUpAsync;
        accountUI.EnterSpaceButtonClicked += EnterSpaceAsync;
        accountUI.CreateSpaceButtonClicked += CreateSpaceAsync;
        accountUI.DeleteSpaceButtonClicked += DeleteSpaceAsync;
        accountUI.LogoutButtonClicked += LogoutAsync;
    ...
    private async void SignInAsync(string email, string password)
    {
        ...
        if (result.Item1)
        {
            ...
            accountUI.SignInSucceeded();
        }
        else
        {
            accountUI.SignInFailed(result.Item2);
        }
    }
    ...
    /// <returns> Just the Task object to await.</returns>
    private async Task<(bool, string)> LoginAsync(string email, string password)
    {
        ...
        {
            Debug.LogError($"Failed to log in - {loginResult.GetHttpResultCode()} {loginResult.GetResponseBody()}");
            return (false, $"{loginResult.GetHttpResultCode()} {loginResult.GetResponseBody()}");
        }
    }
    ...
    private async void CreateSpaceAsync(string spaceName)
    {
        ...
        if (spaceResult.GetResultCode() == Csp.Systems.EResultCode.Success)
        {
            ...
            accountUI.CreateSpaceSucceeded(createdSpace.Id);
        }
        else
        {
            Debug.LogError($"Failed to create space - {spaceResult.GetHttpResultCode()} {spaceResult.GetResponseBody()}");
        }
    }

    /// <summary>
    /// Enters the space with the given spaceId.
    /// </summary>
    /// <param name="spaceId">The destination space's ID.</param>
    private async void EnterSpaceAsync(string spaceId)
    {
