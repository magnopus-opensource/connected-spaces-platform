...
export async function runTests() {
    ...
    console.log(`${C('[==========]', CONSOLE_GREEN)} Running ${totalTestCount} test${(totalTestCount > 1 ? 's' : '')}`);
    ...
    const /** @type {XMLElement} */ suiteResults = results.createElement('testsuite');
    let suiteElapsed = 0;
    ...
    for (const test of testsCollection) {        
        const /** @type {XMLElement} */ testResult = suiteResults.createElement('testcase');
        ...
        try {
            ...
            const /** @type {XMLElement} */ testFailure = testResult.createElement('failure');
        ...
        if (passed) {
            console.log(`${C('[       OK ]', CONSOLE_GREEN)} ${test.name} (${elapsed} ms)`);
            passedCount++;
        }
        else {
            console.log(`${C('[     FAIL ]', CONSOLE_RED)} ${test.name} (${elapsed} ms)`);
    ...
    console.log();
    console.log(`${C('[==========]', CONSOLE_GREEN)} ${totalTestCount} test${(totalTestCount > 1 ? 's' : '')} ran (${totalElapsed} ms total)`);
