...
def find_unity_installation():
    ...
    result = subprocess.run([get_unity_hub_path(), '--', '--headless', 'editors', '-i'], shell=True, capture_output=True, text=True)
        ...
        f.write('''
{
  ...
  "noEngineReferences": false
    ...
    subprocess.run([editor_path, '-runTests', '-batchmode', '-projectPath', project_path, '-testResults', args.test_output_path, '-testPlatform', 'PlayMode', '-nographics'], shell=True)
    print("Done!")
